...
; CSE's NormalStep takes the object action in d (see Movement_do_step);
; without it the follower would get action 0 (SetFacingStanding) and vanish while stepping.
	pop af
	push af
...
if 0
	...
	ld d, [hl]
	ld [hl], movement_step_sleep ; no-optimize *hl++|*hl-- = N
...
.PokeballOpeningObject:
	...
	db $6c | $80, PAL_OW_RED, SPRITEMOVEDATA_POKEBALL_OPENING ; bit 7 selects VRAM bank 0; base tile $6c
...
.PokeballClosingObject:
	; vtile, palette, movement
	db $6c | $80, PAL_OW_RED, SPRITEMOVEDATA_POKEBALL_CLOSING ; bit 7 selects VRAM bank 0; base tile $6c
...
; face the follower the same way, if it's loaded
	ld a, FOLLOWER
