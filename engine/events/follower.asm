...
MACRO interaction
	dw \1 ; species (16-bit index, -1 = any)
	db \2 ; type inclusion
	db \3 ; type exclusion
	db \4 ; landmark
	db \5 ; map id
...
DEF FOLLOWER_INT_TABLE_ROW_LENGTH EQU 12
...
DoFollowerInteraction:
	...
	ld c, l ; bc/d = 16-bit species index (b is reused as the row byte counter)
	ld d, h
	ld hl, FollowerInteractionTable
.get_row
	ld b, FOLLOWER_INT_TABLE_ROW_LENGTH
; species (dw); -1 = any. No real index has a $ff high byte.
	ld a, [hli]
	ld e, a
	ld a, [hli]
	dec b
...
; sliding on ice until the next map load runs RefreshFollowingCoords. Put both back.
	xor a
...
DefaultInteraction:
; The jump uses the follower's walking frames and spawns a shadow. Those tiles live in
; vTiles1, which holds the font while a textbox is open, so animate before opentext.
	followcry
	random 50
	ifequal 1, .give_item
